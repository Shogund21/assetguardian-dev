...
BEGIN
  ...
  LOOP
    ...
    -- Open WO count (unchanged)
    SELECT count(*) INTO v_open_wo
    FROM public.automated_work_orders
    WHERE asset_id = rec.id
      AND status NOT IN ('completed', 'closed');

    -- Corrective WO count: last 12 months, matching corrective keywords or high/urgent priority
    SELECT count(*) INTO v_corrective_wo
    FROM public.automated_work_orders
    WHERE asset_id = rec.id
      AND created_at >= now() - interval '12 months'
      AND (
        lower(title) ~ '(repair|leak|alarm|fault|failure|replace|no cool|no cooling)'
        OR lower(description) ~ '(repair|leak|alarm|fault|failure|replace|no cool|no cooling)'
        OR lower(priority) IN ('high', 'urgent')
      );
